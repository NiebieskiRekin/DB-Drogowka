...
*/
create or replace function create_and_return_text_message 
   (p_constraint_name varchar2
   ) return varchar2 is
   PRAGMA AUTONOMOUS_TRANSACTION; 
    l_message varchar2(4000) :=   'TODO: Provide better message in APEX Shared Components Text Messages for ' || p_constraint_name ;
begin
   apex_lang.create_message
      (p_application_id  => v('APP_ID'),
       p_name            => p_constraint_name,
       p_language        => nvl(apex_util.get_preference('FSP_LANGUAGE_PREFERENCE'), 'pl'),
...
create or replace function error_handler 
...
begin
    ...
    if p_error.is_internal_error then
        ...
        if not p_error.is_common_runtime_error then
            ...
            l_result.message         := 'Nastąpił nieoczekiwany błąd o identyfikatorze:  '||to_char(l_reference_id, '999G999G999G990');
